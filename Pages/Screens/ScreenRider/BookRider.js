...
const BookRider = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                setErrorMsg('Error getting location or geocoding addresses');
            }
        })();
    }, [rideDetails]);
    ...
    useEffect(() => {
        if (pickupCoords && destinationCoords) {
            ...
            // Calculate the delta to ensure both points are visible with padding
            const latDelta = Math.abs(pickupCoords.latitude - destinationCoords.latitude) * 2;
            const lngDelta = Math.abs(pickupCoords.longitude - destinationCoords.longitude) * 2;
